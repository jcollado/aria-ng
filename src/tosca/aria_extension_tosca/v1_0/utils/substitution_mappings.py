...
def validate_subtitution_mappings_requirement(context, presentation):
    ...
    for n, the_requirement in requirements:
        if n == presentation._name:
            type_requirement = the_requirement
            break
    if type_requirement is None:
        context.validation.report('substitution mappings requirement "%s" is not declared in node type "%s"' % (presentation._name, node_type._name), locator=presentation._locator, level=Issue.BETWEEN_TYPES)
        return
...
def validate_subtitution_mappings_capability(context, presentation):
    ...
    if type_capability is None:
        context.validation.report('substitution mappings capability "%s" is not declared in node type "%s"' % (presentation._name, node_type._name), locator=presentation._locator, level=Issue.BETWEEN_TYPES)
        return
